...
<div class="space-y-6 max-w-xl">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Edit Karyawan</h1> 
            <p class="text-xs text-slate-500 mt-0.5">Perbarui informasi jabatan, alamat, atau no handphone karyawan.</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 lg:p-8">
        <form action="/karyawan/{{ $karyawan->id }}" method="POST" class="space-y-5"> 
            @csrf @method('PUT') 
            
            <!-- User Display (Readonly style) -->
            <div>
                <label class="block text-sm font-semibold text-slate-400 mb-2">Nama Karyawan</label>
                <div class="block w-full px-5 py-3.5 bg-slate-100 border border-slate-200 rounded-2xl text-slate-500 font-bold select-none cursor-not-allowed">
            ...
            <div>
                ...
                <input type="text" name="alamat" id="alamat" value="{{ $karyawan->alamat }}" required class="block w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition" />
            </div>

            <!-- No HP -->
            <div>
                <label for="no_hp" class="block text-sm font-semibold text-slate-600 mb-2">Nomor Telepon / HP</label>
                <input type="text" name="no_hp" id="no_hp" value="{{ $karyawan->no_hp }}" required class="block w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition" />
            </div>
