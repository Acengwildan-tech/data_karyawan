...
<div class="space-y-6 max-w-xl">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Edit Jabatan</h1> 
            <p class="text-xs text-slate-500 mt-0.5">Ubah nama dan informasi jabatan yang sudah terdaftar.</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 lg:p-8">
        <form action="/jabatan/{{ $jabatan->id }}" method="POST" class="space-y-6"> 
            @csrf @method('PUT') 
            
            <div>
                <label for="nama_jabatan" class="block text-sm font-semibold text-slate-600 mb-2">Nama Jabatan</label>
                <input type="text" name="nama_jabatan" id="nama_jabatan" value="{{ $jabatan->nama_jabatan }}" required class="block w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition" />
            </div>
